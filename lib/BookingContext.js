...
import akbarBookingApi from './akbarBookingApi';

/**
 * akbar Booking Context
 * 
...
export function BookingProvider({ children }) {
  ...
  const searchFlights = async (params) => {
    setLoading(true);
    try {
      const result = await akbarBookingApi.searchFlights(params);
      setState(prev => ({
        ...prev,
        loading: false,
        searchParams: params,
  ...
  const startBooking = async () => {
    ...
    setLoading(true);
    try {
      const result = await akbarBookingApi.startBooking(
  ...
  const addPassengers = async (passengers) => {
    ...
    try {
      ...
      const result = await akbarBookingApi.addPassengers(
  ...
  const holdFlight = async () => {
    ...
    setLoading(true);
    try {
      const result = await akbarBookingApi.holdFlight(state.orderReference);
      setState(prev => ({
        ...prev,
        loading: false,
        bookingStatus: 'held',
  ...
  const processPayment = async (paymentData) => {
    ...
    setLoading(true);
    try {
      const result = await akbarBookingApi.processPayment(
        state.orderReference,
        {
          ...paymentData,
  ...
  const confirmBooking = async () => {
    ...
    setLoading(true);
    try {
      const result = await akbarBookingApi.confirmBooking(state.orderReference);
      setState(prev => ({
        ...prev,
        loading: false,
  ...
  const issueTicket = async () => {
    ...
    setLoading(true);
    try {
      const result = await akbarBookingApi.issueTicket(state.orderReference);
      setState(prev => ({
        ...prev,
        loading: false,
  ...
  const getBookingDetails = async (orderRef = null) => {
    ...
    setLoading(true);
    try {
      const result = await akbarBookingApi.getBookingDetails(reference);
      if (!orderRef) {
        setState(prev => ({
          ...prev,
          loading: false,
  ...
  const value = {
    ...
    isAuthenticated: !!akbarBookingApi.getAuthToken(),

    // Auth actions
    login: akbarBookingApi.login,
    register: akbarBookingApi.register,
    logout: () => {
      akbarBookingApi.logout();
      resetBooking();
    },
