/**
 * NDC Booking API Client
 * 
 * Single source of truth for all NDC booking API calls.
...
export const searchFlights = async (params) => {
  ...
  const isTestMode = process.env.NEXT_PUBLIC_NDC_TEST_MODE === 'true';
  const endpoint = isTestMode 
    ? '/local-test/ndc/search'  // Mock API for testing
    : '/ndc/flights/available-offers';  // Real NDC API
...
export const getFlightBundles = async (offerId) => {
  const isTestMode = process.env.NEXT_PUBLIC_NDC_TEST_MODE === 'true';
  const endpoint = isTestMode
    ? `/local-test/ndc/bundles/${offerId}`
    : `/ndc/flights/bundle-options`;
...
export const startBooking = async (offerId, flightData) => {
  return apiRequest('/v2/ndc/bookings/start', {
    method: 'POST',
    body: JSON.stringify({
      offer_id: offerId,
...
export const addPassengers = async (orderReference, passengers) => {
  return apiRequest('/v2/ndc/bookings/passengers', {
    method: 'POST',
    body: JSON.stringify({
...
export const holdFlight = async (orderReference) => {
  return apiRequest('/v2/ndc/bookings/hold', {
    method: 'POST',
    body: JSON.stringify({
      order_reference: orderReference,
    }),
...
export const processPayment = async (orderReference, paymentData) => {
  return apiRequest('/v2/ndc/bookings/pay', {
    method: 'POST',
    body: JSON.stringify({
      order_reference: orderReference,
...
export const issueTicket = async (orderReference) => {
  return apiRequest('/v2/ndc/bookings/issue-ticket', {
    method: 'POST',
    body: JSON.stringify({
      order_reference: orderReference,
    }),
...
export const getBookingDetails = async (orderReference) => {
  return apiRequest(`/v2/ndc/bookings/${orderReference}`, {
    method: 'GET',
  });
};
...
export const listBookings = async (params = {}) => {
  ...
  return apiRequest(`/v2/ndc/bookings${queryString ? '?' + queryString : ''}`, {
    method: 'GET',
  });
};
...
export const cancelBooking = async (orderReference, reason) => {
  return apiRequest('/v2/ndc/bookings/cancel', {
    method: 'POST',
    body: JSON.stringify({
      order_reference: orderReference,
      reason: reason,
