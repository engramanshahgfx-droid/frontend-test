...
// Default VAT rate (Saudi Arabia current standard)
export const VAT_RATE = 0.15;

/**
 * Calculate amount including VAT
 * @param {number} amount - base amount
 * @param {number} rate - VAT rate (e.g. 0.15 for 15%)
 * @returns {number} - amount including VAT
 */
export function amountWithVAT(amount, rate = VAT_RATE) {
  const a = Number(amount) || 0;
  return +(a * (1 + (Number(rate) || 0))).toFixed(2);
}
